...
def vista_tabla_clientes(page, mostrar_detalles_cliente, color_letras="#000000", color_tematica="#E8E8E8"):
    """
    Genera la tabla de clientes consultando desde la base de datos.
    """
    try:
        conn = conectar_db()
        ...
        return ft.Text("Error al cargar datos de clientes.", color="red")
    finally:
        if conn:
            conn.close()
    ...
    if not clientes:
        return ft.Text("No hay clientes registrados.", color="red", size=16)

    def crear_boton_detalles(cliente):
        """
        Crear el botón 'Ver Detalles' para cada cliente.
        """
        return ft.IconButton(
            icon=ft.Icons.VISIBILITY,
            tooltip="Ver Detalles",
            icon_color=color_tematica,
            on_click=lambda e: mostrar_detalles_cliente(dict(cliente))  # Convertir Row a diccionario
        )
