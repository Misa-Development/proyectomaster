...
def inicializar_tabla_ingresos(color_letras):
    ...
    rows = [
        ...
        for ingreso in reversed(obtener_ingresos())  # <-- Cambiado aquí
    ]
    return rows
def tabla_ingresos():
    """
    ...
    return ft.DataTable(
        columns=[            
            ...
            ft.DataColumn(ft.Text("Cantidad", color=color_letras)), 
            ft.DataColumn(ft.Text("Fecha", color=color_letras)),
            ft.DataColumn(ft.Text("Método de Pago", color=color_letras)), # Columna añadida
        ],
